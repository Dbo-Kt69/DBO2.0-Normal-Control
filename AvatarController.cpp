...
void CAvatarController::MouseUpLBtnHandler(const SMouseData *pData)
{
	...
	// mouse click 
	if(pData->uiDownDeltaTime <= 300 && m_iDeltaLButtonMouseX < 10 && m_iDeltaLButtonMouseY < 10)
	{
		if (Logic_IsDirectMode(pSobAvatar))
			return;
...
//
//	RwBool bChangeHeadingEnable = TRUE;
//
//	if (m_bAutoRun)
//	{
...
//					CNtlSLEventGenerator::KeyboardMove(uiMoveFlags, FALSE, fAngleY);
...
//
//
//		m_fChangeHeadingDeltaAngleX += iDeltaX * NTL_MOUSE_SENSITIVITY_SCALE;
//		m_fChangeHeadingDeltaAngleY += iDeltaY * NTL_MOUSE_SENSITIVITY_SCALE;
